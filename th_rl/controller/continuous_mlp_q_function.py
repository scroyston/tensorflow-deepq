    ...
    def __init__(
        ...
        for idx, size in enumerate(hidden_sizes):
            ...
            l_hidden = L.DenseLayer(l_hidden, num_units=size, W=hidden_W_init, b=hidden_b_init,
                                    nonlinearity=hidden_nonlinearity, name="h%d" % (idx + 1))

        if action_merge_layer == n_layers:
            l_hidden = L.ConcatLayer([l_hidden, l_action])

        l_output = L.DenseLayer(l_hidden, num_units=1, W=output_W_init, b=output_b_init,
                                nonlinearity=output_nonlinearity, name="output")

        output_var = L.get_output(l_output, deterministic=True).flatten()

        self._f_qval = theano.function(inputs=[l_obs.input_var, l_action.input_var], outputs=output_var,
