...
def compile_function(inputs=None, outputs=None, updates=None, givens=None, **kwargs):
    import theano
    return theano.function(inputs=inputs, outputs=outputs, updates=updates, givens=givens, on_unused_input='ignore',
                           allow_input_downcast=True, **kwargs)


def parse_update_method(update_method, **kwargs):
    if update_method == 'adam':
        return partial(lasagne.updates.adam, **kwargs)
    elif update_method == 'sgd':
        return partial(lasagne.updates.sgd, **kwargs)
    else:
        raise NotImplementedError
...
class DDPG(object):
    ...
    def train(self):
        ...
        pool = SimpleReplayPool(max_pool_size=self.replay_pool_size,
    ...
    def init_opt(self):
        ...
        qf_weight_decay_term = 0.5 * self.qf_weight_decay * \
                               sum([TT.sum(TT.square(param)) for param in self.qf.get_params(regularizable=True)])
        ...
        f_train_qf = compile_function(inputs=[yvar, obs, action], outputs=[qf_loss, qval], updates=qf_updates)

        f_train_policy = compile_function(inputs=[obs], outputs=policy_surr, updates=policy_updates)

        self.opt_info = dict(f_train_qf=f_train_qf, f_train_policy=f_train_policy,
                             target_qf=target_qf, target_policy=target_policy, )

    def do_training(self, batch):
        obs, actions, rewards, next_obs, terminals = extract(batch, "observations", "actions", "rewards",
